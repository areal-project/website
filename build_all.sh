...
uv run --with-requirements "$SCRIPT_DIR/requirements.txt" jupyter-book build "$SCRIPT_DIR/en" --all --path-output "$SCRIPT_DIR/_build/en"

echo "[AReaL] Building Chinese version..."
uv run --with-requirements "$SCRIPT_DIR/requirements.txt" jupyter-book build "$SCRIPT_DIR/zh" --all --path-output "$SCRIPT_DIR/_build/zh"
